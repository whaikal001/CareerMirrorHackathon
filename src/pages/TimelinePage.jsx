import { ArrowLeft, ArrowLeftRight, Target, AlertTriangle } from "lucide-react";
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { SALARY_SOURCE } from "../data/careerData";

export default function TimelinePage({ picked, go, planB }) {
  return (
    <div className="ct-screen" style={{ paddingTop: 30 }}>
      ...
      <div className="ct-card" style={{ marginTop: 22 }}>
        ...
        <div style={{ fontSize: 11.5, color: "var(--faint)", marginTop: 8 }}>{SALARY_SOURCE}</div>
      </div>
      ...
      {planB && <div className="ct-alert" style={{ marginTop: 18, alignItems: "center", flexWrap: "wrap" }}><AlertTriangle size={18} color="var(--amber)" style={{ flex: "none" }} /><div style={{ fontSize: 14.5, flex: "1 1 260px" }}>If {picked.title} gets disrupted, your best pivot is <b>{planB.pivotPath.title}</b>.</div><button className="ct-btn ghost" style={{ padding: "8px 14px", fontSize: 13.5, flex: "none" }} onClick={() => go("dashboard")}>View Plan B</button></div>}
      <div style={{ marginTop: 24, display: "flex", gap: 12, flexWrap: "wrap" }}><button className="ct-btn ghost" onClick={() => go("twin")}><ArrowLeft size={17} /> Paths</button><button className="ct-btn ghost" onClick={() => go("whatif")}><ArrowLeftRight size={17} /> Compare paths →</button><button className="ct-btn pri" onClick={() => go("skillgap")}><Target size={17} /> Analyze skill gap</button></div>
    </div>
  );
}
