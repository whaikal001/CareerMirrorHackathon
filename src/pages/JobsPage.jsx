...
export default function JobsPage({ go, startRehearsal, jobMatches }) {
  const [sector, setSector] = useState("");
  const [loc, setLoc] = useState("");
  const [type, setType] = useState("");
  const jobs = JOBS.map((j) => ({ ...j, match: jobMatches?.[j.id] ?? j.match })).sort((a, b) => b.match - a.match);
  ...
  return (
    <div className="ct-screen" style={{ paddingTop: 30 }}>
      ...
      <div className="ct-mono" style={{ marginTop: 14, fontSize: 12.5, color: "var(--faint)", letterSpacing: 1.5 }}>{filtered.length} OF {JOBS.length} ROLES · SORTED BY YOUR MATCH</div>
      {filtered.length ? (
